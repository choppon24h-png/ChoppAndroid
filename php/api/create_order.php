...
if (!in_array($payment_method, $allowed_methods)) {
    http_response_code(400);
    echo json_encode(['error' => "payment_method inválido. Use: pix, debit ou credit"]);
    exit;
}
...
// ── Alerta de Venda por E-mail (opcional) ─────────────────────
try {
...
if ($payment_method === 'pix') {
    ...
    if ($result) {
        ...
        echo json_encode(['error' => 'Erro ao criar checkout PIX. Verifique a configuração SumUp.']);
    }
    exit;
}
...
try {
    ...
    // Tabela readers pode não existir — não crítico
}
...
if ($result) {
    ...
    http_response_code(500);
    echo json_encode([
        'error'     => 'Erro ao criar checkout de cartão. Verifique se a leitora está online e as configurações SumUp (affiliate_key, affiliate_app_id).',
        'card_type' => $payment_method,
        'reader_id' => $tap['reader_id'],
    ]);
}
